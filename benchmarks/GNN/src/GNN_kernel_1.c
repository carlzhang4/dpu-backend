...
int main() {
    ...
    uint32_t max_cols_per_dpu_w = DPU_INPUT_ARGUMENTS_A.max_cols_per_dpu;
    ...
    uint32_t mram_base_addr_mid = (uint32_t) (DPU_MRAM_HEAP_POINTER + 2 * max_nnz_per_dpu_A * sizeof(struct elem_t) + max_cols_per_dpu_w * tcols_feat * sizeof(T));
    ...
    barrier_wait(&tasklet_16_barrier);
        ...
        //initialize sequential_reader for nnzs
