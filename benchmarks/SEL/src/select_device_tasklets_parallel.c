...
int main_kernel1() {
    ...
    for(unsigned int byte_index = base_tasklet; byte_index < input_size_dpu_bytes; byte_index += BLOCK_SIZE * TOTAL_TASKLETS){
        ...
        for(unsigned int i = 0; i < l_count; i++){
            //printf("tasklet_id = %u, mram value[%u] = %lu\n", tasklet_id, (message_partial_count + p_count + i), cache_B[i]);
        }
        
        //printf("tasklet_id = %u, write to mram addr = %u, write size = %u\n", tasklet_id, mram_base_addr_B + (message_partial_count + p_count) * sizeof(T), l_count * sizeof(T));
        //printf("offset compared to DPU_MRAM_HEAP_POINTER = %u\n", (mram_base_addr_B + (message_partial_count + p_count) * sizeof(T)) - (uint32_t)DPU_MRAM_HEAP_POINTER);
