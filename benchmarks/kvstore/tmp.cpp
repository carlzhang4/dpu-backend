...
#ifndef DPU_BINARY_USER2
#define DPU_BINARY_USER2 "./build/benchmarks/kvstore/kvstore_get_device_hash2"
#endif

#ifndef DPU_BINARY_USER3
#define DPU_BINARY_USER3 "./build/benchmarks/kvstore/kvstore_get_device_hash3"
#endif
...
void thread_KVStore_server(int thread_index, QpHandler *handler, void *buf, size_t ops,NetParam net_param) {
	...
	DPU_ASSERT(dpu_load(set2, DPU_BINARY_USER2, NULL));

	DPU_ASSERT(dpu_alloc(DPU_NUM, "nrThreadPerPool=8", &set3));
	DPU_ASSERT(dpu_load(set3, DPU_BINARY_USER3, NULL));
	...
	uint64_t t[4];
	for (uint64_t i = 0; i < ops; i++) {
			char tmp1[VALUE_SIZE],tmp2[VALUE_SIZE],tmp3[VALUE_SIZE];
			memset(tmp1, 0, VALUE_SIZE);
			memset(tmp2, 0, VALUE_SIZE);
			memset(tmp3, 0, VALUE_SIZE);
			...
			DPU_ASSERT(dpu_push_xfer(set1, DPU_XFER_TO_DPU, "request_key",0, sizeof(uint64_t) , DPU_XFER_ASYNC));

			DPU_FOREACH(set2, dpu, each_dpu){
				DPU_ASSERT(dpu_prepare_xfer(dpu,&request_key));
			}
			DPU_ASSERT(dpu_push_xfer(set2, DPU_XFER_TO_DPU, "request_key",0, sizeof(uint64_t) , DPU_XFER_ASYNC));

			DPU_FOREACH(set3, dpu, each_dpu){
				DPU_ASSERT(dpu_prepare_xfer(dpu,&request_key));
			}
			DPU_ASSERT(dpu_push_xfer(set3, DPU_XFER_TO_DPU, "request_key",0, sizeof(uint64_t) , DPU_XFER_ASYNC));
			...
			DPU_ASSERT(dpu_push_xfer(set1, DPU_XFER_FROM_DPU, "result_value",0, VALUE_SIZE , DPU_XFER_ASYNC));

			DPU_FOREACH(set2, dpu, each_dpu){
				DPU_ASSERT(dpu_prepare_xfer(dpu,tmp2));
			}
			DPU_ASSERT(dpu_push_xfer(set2, DPU_XFER_FROM_DPU, "result_value",0, VALUE_SIZE , DPU_XFER_ASYNC));

			DPU_FOREACH(set3, dpu, each_dpu){
				DPU_ASSERT(dpu_prepare_xfer(dpu,tmp3));
			}
			DPU_ASSERT(dpu_push_xfer(set3, DPU_XFER_FROM_DPU, "result_value",0, VALUE_SIZE , DPU_XFER_ASYNC));
			DPU_ASSERT(dpu_sync(set1));
			DPU_ASSERT(dpu_sync(set2));
			DPU_ASSERT(dpu_sync(set3));
			for(int j = 0; j < VALUE_SIZE; j++) {
				resp->value[j] = tmp1[j] | tmp2[j] | tmp3[j];
			}
			t[3] = get_tscp();
