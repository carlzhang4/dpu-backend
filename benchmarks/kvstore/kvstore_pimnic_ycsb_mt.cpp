...
// 	char key[KEY_SIZE_PIMNIC];
// };

// struct response_message
// {
// 	uint64_t if_valid; // 1 for valid, 0 for invalid
// 	char value[VALUE_SIZE_PIMNIC];
// };
...
void thread_get_client(int thread_index, NetParam &net_param, QpHandler *qp_handler, int *slice_id_array, int *dpu_array, int total_dpu_num, size_t ops, size_t *field_length_array,volatile void **bufs, volatile void **recv_bufs) {
	...
	int warm_up =5;
	int key_size_pimnic = 64*512;
	int value_size_pimnic = 0;
	...
	for (uint64_t i = 0; i < ops/512; i++) {
		...
		std::cout << "start setting magic number for key" << std::endl;
		for(int j=0;j<total_dpu_num;j++){
			std::cout << "setting dpu " << j << " address : " << std::hex << get_dpu_addr(slice_id_array[j],dpu_array[j],MAGIC_OFFSET) << std::dec << std::endl;
			((char**)bufs)[thread_index][get_dpu_addr(slice_id_array[j],dpu_array[j],MAGIC_OFFSET)] = magic_number;
		...
		//* write for rsp magic number
		for(int j=0;j<total_dpu_num;j++){
			std::cout << "setting dpu " << j << " address : " << std::hex << get_dpu_addr(slice_id_array[j],dpu_array[j],VALUE_MAGIC_OFFSET) << std::dec << std::endl;
			((char**)bufs)[thread_index][get_dpu_addr(slice_id_array[j],dpu_array[j],VALUE_MAGIC_OFFSET)] = magic_number;
			//  std::cout <<"set dpu "<<j<<" address : "<<std::hex<<(void*)get_dpu_addr(slice_id_array[j],dpu_array[j],VALUE_MAGIC_OFFSET)<<" to "<<(int)((char**)bufs)[0][get_dpu_addr(slice_id_array[j],dpu_array[j],VALUE_MAGIC_OFFSET)]<<std::dec<<std::endl;
		}
		...
		int cmpt_wqe_cnt = 0;
		// for(uint64_t j= 0x200400;j< 0x200480;j++){
		// 	std::cout << std::hex<<(void*)j<<" : "<<(int)((char**)bufs)[0][j]<<std::dec<<std::endl;
		// }
		if(value_size_pimnic > key_size_pimnic){
			while(cmpt_dpu_cnt < total_dpu_num){
				int start_dpu = cmpt_dpu_cnt;
				post_send(*qp_handler, get_dpu_addr(slice_id_array[start_dpu],dpu_array[start_dpu],VALUE_MAGIC_OFFSET), 8 *16);
		...
		//std::cout << "end post send dpu for value magic number" << std::endl;
		cmpt_dpu_cnt = 0;
		t1 = get_tscp();
		while(cmpt_dpu_cnt < total_dpu_num){
			// std::cout << "total_dpu_num" <<total_dpu_num <<std::endl;
			...
			if(total_dpu_num - cmpt_dpu_cnt >= 16){
				...
				while(write_size < MAGIC_OFFSET +8){
					// std::cout <<"post send dpu from "<<start_dpu<<" to "<<start_dpu+15<<" address : "<<std::hex<<(void*)get_dpu_addr(slice_id_array[start_dpu],dpu_array[start_dpu],request_offset+write_size)<<" size "<<16*8<<std::dec<<std::endl;
					if(MAGIC_OFFSET + 8 - write_size < 8192){
						post_send(*qp_handler, get_dpu_addr(slice_id_array[start_dpu],dpu_array[start_dpu],request_offset+write_size), MAGIC_OFFSET*16 +128 - write_size *16);
						
						// std::cout <<"post send last dpu from "<<start_dpu<<" to "<<start_dpu+15<<" address : "<<std::hex<<(void*)get_dpu_addr(slice_id_array[start_dpu],dpu_array[start_dpu],request_offset+write_size)<<" size "<<MAGIC_OFFSET*16 +128 - 16*write_size<<std::dec<<std::endl;
						write_size += (MAGIC_OFFSET + 8 - write_size);
					}else{
						post_send(*qp_handler, get_dpu_addr(slice_id_array[start_dpu],dpu_array[start_dpu],request_offset+write_size), 8192*16);
						
						// std::cout <<"post send dpu from "<<start_dpu<<" to "<<start_dpu+15<<" address : "<<std::hex<<(void*)get_dpu_addr(slice_id_array[start_dpu],dpu_array[start_dpu],request_offset+write_size)<<" size "<<8192*16<<std::dec<<std::endl;
						write_size += 8192;
					}
					...
					if(outstanding_rdma_wqe >= RDMA_OUTSTANDING_LIMIT){
						int ne = 0;
						while(ne == 0){
							ne = poll_send_cq(*qp_handler, wc_send);
						}
				...
				while(send_size < MAGIC_OFFSET+8){
					...
					post_send(*qp_handler, get_dpu_addr(slice_id_array[start_dpu],dpu_array[start_dpu],request_offset+send_size), 64);
				...
				while(send_size < MAGIC_OFFSET+8){
					...
					post_send(*qp_handler, get_dpu_addr(slice_id_array[start_dpu],dpu_array[start_dpu],request_offset+send_size), remain_dpu);
					// std::cout <<"post send dpu from "<<start_dpu<<" to "<<start_dpu+remain_dpu-1<<" address : "<<std::hex<<(void*)get_dpu_addr(slice_id_array[start_dpu],dpu_array[start_dpu],request_offset+send_size)<<" size "<<remain_dpu*8<<std::dec<<std::endl;
					send_size += 1;
					wqe_cnt++;
					outstanding_rdma_wqe++;
		...
		//std::cout <<"all requests posted, wqe_cnt "<<wqe_cnt<<std::endl;
		while(cmpt_wqe_cnt < wqe_cnt){
			int ne = poll_send_cq(*qp_handler, wc_send);
		...
		volatile char* value_buf = (volatile char*)recv_bufs[thread_index];
		//std::cout << "start polling dpu result" << std::endl;
		while(!all_done){
			// std::cout <<"polling dpu result, magic_number "<<(int)magic_number<<std::endl;
			for(int j=0;j<total_dpu_num;j++){
				resp_valid[j] = value_buf[get_dpu_addr(slice_id_array[j],dpu_array[j],VALUE_MAGIC_OFFSET)];
				// resp_valid[j] = value_buf[64*(j/8) + j%8];
				// std::cout <<"read dpu "<<j<<" address : "<<std::hex<<(void*)get_dpu_addr(slice_id_array[j],dpu_array[j],VALUE_MAGIC_OFFSET)<<" value "<<(int)resp_valid[j]<<std::dec<<std::endl;
			}
			all_done = true;
			for(int j=0;j<total_dpu_num;j++){
...
void benchmark_KVStore_client(NetParam *net_param_ptr) {
	...
	for (size_t i = 0; i < 10000 && field_length_file >> field_length_array[i]; ++i);
	...
	size_t **field_length_array_array = new size_t*[NUM_THREADS]();
	for(int i = 0;i < NUM_THREADS;i++){
		field_length_array_array[i] = new size_t[ops/REQUEST_PER_DPU]();
		for(int j = 0;j < ops/REQUEST_PER_DPU;j++){
			field_length_array_array[i][j] = field_length_array[j%10000];
		}
	}

	for(size_t i = 0; i < ops/REQUEST_PER_DPU; ++i){
		std::cout << field_length_array_array[0][i] << " ";
	}
	...
	volatile void **bufs = new volatile void *[NUM_THREADS];
	volatile void **recv_bufs = new volatile void *[NUM_THREADS];
	QpHandler **qp_handlers = new QpHandler * [NUM_THREADS]();
	// int total_dpu_num;	
	// int slice_id_array[MAX_DPU_NUM];
	// int dpu_array[MAX_DPU_NUM];
	int slice_id_array[MAX_THREADS][MAX_DPU_NUM];
	int dpu_array[MAX_THREADS][MAX_DPU_NUM];
	int total_dpu_num[MAX_THREADS];

	for (int i = 0;i < NUM_THREADS;i++) {
		bufs[i] = malloc_2m_numa(BUF_SIZE, net_param_ptr[i].numa_node);
		memset((void*)bufs[i],0,BUF_SIZE);
		recv_bufs[i] = malloc_2m_numa(BUF_SIZE, net_param_ptr[i].numa_node);
		memset((void*)recv_bufs[i],0,BUF_SIZE);
	...
	for (int i = 0;i < NUM_THREADS;i++) {
		std::cout << "create qp for thread " << i << std::endl;
		
		int my_id   = net_param_ptr[i].nodeId % net_param_ptr[i].numNodes;
        int dest_id = (net_param_ptr[i].nodeId + 1) % net_param_ptr[i].numNodes;
		qp_handlers[i] = create_qp_rc(net_param_ptr[i], (void*)(bufs[i]), BUF_SIZE, info + 2*i + my_id, 0);
		
        size_t bytes_write = write(net_param_ptr[i].sockfd[0], info + my_id + 2*i, sizeof(PingPongInfo) );
		if(bytes_write != sizeof(PingPongInfo)){
			std::cout << "bytes_write != sizeof(PingPongInfo)" << std::endl;
		}
		read(net_param_ptr[i].sockfd[0], &(info[dest_id + 2*i]), sizeof(PingPongInfo) );
	...
	//* send the zero-initialized send buffer to DPU DRAM to clear stale data
	for(int i = 0;i < NUM_THREADS;i++){
		struct ibv_wc *wc_init = NULL;
		ALLOCATE(wc_init, struct ibv_wc, CTX_POLL_BATCH);
		post_send(*qp_handlers[i], 0, BUF_SIZE);
		while(!poll_send_cq(*qp_handlers[i], wc_init));
		free(wc_init);
		std::cout << "Thread " << i << ": initial post_send done (DPU DRAM cleared)" << std::endl;
	}

	//* establish DMA connection with server BF
	for(int i = 0;i < NUM_THREADS;i++){
		export_client_memory(net_param_ptr[i], recv_bufs[i], BUF_SIZE, 1);
		std::cout << "recv_bufs[" << i << "] Address : "<<recv_bufs[i]<<std::endl;
	}
	...
	for(int i = 0;i < NUM_THREADS;i++){
		recv(net_param_ptr[i].sockfd[0], &total_dpu_num[i], sizeof(int), 0); 
		std::cout << "total_dpu_num: " << total_dpu_num[i] << std::endl;
		for(int j = 0;j < total_dpu_num[i];j++){
			recv(net_param_ptr[i].sockfd[0], &(slice_id_array[i][j]),sizeof(int), 0);
			recv(net_param_ptr[i].sockfd[0], &(dpu_array[i][j]),sizeof(int), 0);
		}
	}
	std::cout << "end receive dpu info from server!"<<std::endl;
	getchar();
	...
	for(int i = 0;i < NUM_THREADS;i++){
		...
		threads[i] = thread(thread_get_client, i, std::ref(net_param_ptr[i]), qp_handlers[i], slice_id_array[i], dpu_array[i], total_dpu_num[i], ops, field_length_array_array[i], bufs, recv_bufs);
	...
	char end_buf[10];
	send(net_param_ptr[0].sockfd[0], end_buf, sizeof(end_buf), 0);
	memset(end_buf, 0, sizeof(end_buf));
	std::cout << "end send end signal to server!" << std::endl;
	std::cout << "total duration: " << (double)(d1)/2.1/1000/(ops) << "us" << std::endl;
...
void benchmark_KVStore_server(NetParam *net_param_ptr) {
	size_t BUF_SIZE = 760*1024*1024;
	if(BUF_SIZE <= 4096){
		BUF_SIZE = 4096*2;
	}
	...
	// }
	// //* all threads share the same DPU memory region
	// for(int i = 1; i < NUM_THREADS; i++){
	// 	bufs[i] = bufs[0];
	// }
	for(int i = 0; i < NUM_THREADS; i++){
		bufs[i] = malloc_2m_numa(BUF_SIZE*2, net_param_ptr[i].numa_node);
	}
	...
	QpHandler **qp_handlers = new QpHandler * [NUM_THREADS]();
	for(int i = 0;i < NUM_THREADS;i++){
		qp_handlers[i] = create_qp_rc(net_param_ptr[i], bufs[i], BUF_SIZE, info + 3*i, 0);
	}
	for(int i = 0;i < NUM_THREADS;i++){
		read(net_param_ptr[i].sockfd[1], &(info[3*i+1]), sizeof(PingPongInfo) );
		write(net_param_ptr[i].sockfd[1], &(info[3*i]), sizeof(PingPongInfo) );
		connect_qp_rc(net_param_ptr[i], *qp_handlers[i], info + 3*i+1 , info + 3*i);
	}
	...
	for(int i = 0;i < NUM_THREADS;i++){
		for(auto it = _ranks.begin(); it != _ranks.end(); ++it){
			...
			rank.open_access();
			// auto addr = rank.base_region_addr;
			auto addr = (void*)bufs[i];
			export_pim(net_param_ptr[i], (void*)addr, 760*1024*1024,1);
		}
	}
	...
	vhca_resource* client_resource = new vhca_resource[NUM_THREADS];
	for(int i = 0;i < NUM_THREADS;i++){
		read(net_param_ptr[i].sockfd[1], &(client_resource[i]),   sizeof(vhca_resource));
		write(net_param_ptr[i].sockfd[2], &(client_resource[i]), sizeof(vhca_resource));
	}
	...
	for(int i = 0;i < NUM_THREADS;i++){
		for(auto it = _ranks.begin(); it != _ranks.end(); ++it){
			...
			// auto addr = rank.base_region_addr;
			auto addr = (void*)bufs[i];
			export_pim(net_param_ptr[i], (void*)addr, 760*1024*1024,2);
	...
	for(int i = 0;i < NUM_THREADS;i++){
		send(net_param_ptr[i].sockfd[1],  &num_dpus, sizeof(int), 0); 
		std::cout << "num_dpus: " << num_dpus << std::endl;
		for(int j = 0;j < num_dpus;j++){
			send(net_param_ptr[i].sockfd[1], &(slice_id_array[j]),sizeof(int), 0);
			send(net_param_ptr[i].sockfd[1], &(dpu_array[j]),sizeof(int), 0);
		}
		std::cout << "slice_id: " << slice_id_array[i] << ", dpu_id: " << dpu_array[i] << std::endl;
	}

	for(int i = 0;i < NUM_THREADS;i++){
		send(net_param_ptr[i].sockfd[2],  &num_dpus, sizeof(int), 0); 
		std::cout << "num_dpus: " << num_dpus << std::endl;
		for(int j = 0;j < num_dpus;j++){
			send(net_param_ptr[i].sockfd[2], &(slice_id_array[j]),sizeof(int), 0);
			send(net_param_ptr[i].sockfd[2], &(dpu_array[j]),sizeof(int), 0);
		}
	}
	...
	read(net_param_ptr[0].sockfd[1], end_buf, sizeof(end_buf));
...
int main(int argc, char *argv[]) {
    ...
    for(int i = 0; i < NUM_THREADS; i++) {
		if(FLAGS_nodeId == 0) {
			net_param_ptr[i].numNodes = 3;   //* server
		} else if (FLAGS_nodeId == 1) {   //* client
			net_param_ptr[i].numNodes = 2;
		} else {
			std::cout << "nodeId should be 0 or 1" << std::endl;
			exit(1);
		}
		net_param_ptr[i].nodeId           = FLAGS_nodeId;
		...
		roce_init(net_param_ptr[i], 3);
		if(net_param_ptr[i].nodeId == 0) {
			send(net_param_ptr[i].sockfd[1], &ITERATIONS, sizeof(int), 0);
			send(net_param_ptr[i].sockfd[2], &ITERATIONS, sizeof(int), 0);
		}
		else if(net_param_ptr[i].nodeId == 1) {
	...
	if(FLAGS_nodeId == 0) {
		benchmark_KVStore_server( net_param_ptr);
	} else if (FLAGS_nodeId == 1) {
		benchmark_KVStore_client( net_param_ptr);
	} else {
		std::cout << "nodeId should be 0 or 1" << std::endl;
		exit(1);
	}
