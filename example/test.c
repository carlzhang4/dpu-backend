...
int test_allgather(uint32_t data_num_per_dpu) {
    ...
    //for(int i=0;i<interval;i++)
    //{
        pidcomm_allgather(hypercube_manager, "110", data_size_per_dpu, start_offset, target_offset, buffer_offset);
    //}
    // t4 = clock();
    // sum = 1.0*(t4-t1)/CLOCKS_PER_SEC * 1000;
    //end_tsc = get_tscp();
...
int test_allreduce(uint32_t data_num_per_dpu) {
    ...
    //for(int i=0;i<interval;i++)
    //{
        pidcomm_all_reduce(hypercube_manager, "110", data_size_per_dpu, start_offset, target_offset, buffer_offset,sizeof(uint32_t),1);
