/* Instrumented copy of the frozen benchmarks/SEL/select_pim.cpp for the
 * V2 equivalence reference.  The original file is never modified.  The
 * only functional differences are marked with "V2REF":
 *  - the DPU binary prefix is a flag so it can be an absolute path;
 *  - the last iteration's concatenated hit array (bufferC, which the
 *    original computes and then discards) is dumped as binary u64
 *    records so it can be compared against the PIMNIC library path. */
#include <iostream>
#include <thread>
#include <mutex>
#include <cstdio>
...
void thread_select_server(int thread_index, QpHandler *handler, void *buf, size_t ops,NetParam net_param) {
    ...
    uint32_t nr_of_dpus;
	/* V2REF: flag instead of the hard-coded relative prefix. */
	string dpu_binary_path = DPU_BINARY_PREFIX + std::to_string(NR_TASKLETS);
	...
	for (uint64_t i = 0; i < ITERATIONS; i++) {
		...
		t6 = get_tscp();
		/* V2REF: dump the concatenated hit array once (every
		 * iteration recomputes identical data). */
		if (i == (uint64_t)(ITERATIONS - 1) && !DUMP_PATH.empty()) {
			std::ofstream dump(DUMP_PATH,
					   std::ios::binary | std::ios::trunc);
			dump.write((const char *)bufferC,
				   (std::streamsize)accum * sizeof(uint64_t));
			std::cout << "V2REF dumped " << accum
				  << " hits to " << DUMP_PATH << std::endl;
		}
...
void benchmark(NetParam &net_param) {
	...
	/* V2REF: the original's 2 GiB per process exceeds the machine's
	 * 2 GiB hugepage pool when server and client share a host; 64 MiB
	 * amply covers 3 iterations of 64000-row input + output and does
	 * not affect DPU results. */
	BUF_SIZE = 64UL*1024*1024;
...
/* V2REF */
DEFINE_string(dpuBinary, DPU_BINARY_USER, "DPU binary path prefix");
DEFINE_string(dumpPath, "", "dump the final hit array to this file");
...
int main(int argc, char *argv[]) {
	...
	/* V2REF */
	DPU_BINARY_PREFIX = FLAGS_dpuBinary;
