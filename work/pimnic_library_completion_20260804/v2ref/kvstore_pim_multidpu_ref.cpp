/* Instrumented copy of the frozen benchmarks/kvstore/kvstore_pim_multidpu.cpp
 * for the V2 equivalence reference.  The original file is never modified.
 * Functional differences are marked with "V2REF":
 *  - the DPU binary path is a flag so it can be absolute;
 *  - the host-side table is heap allocated so the full 32 MiB identity
 *    table (max_hash_entry_num = 2097152 = the DPU's TOTAL_KV_ENTRIES)
 *    can be loaded; the original only initialised a prefix, which left
 *    most SipHash lookups reading undefined MRAM;
 *  - the RDMA buffer is sized from the table + request stream instead of
 *    10x the table (buffer sizing does not affect DPU results);
 *  - every (key, value) response that the original read back and then
 *    discarded is dumped as binary u64 pairs, and the three dpu_set
 *    copies are cross-checked. */
#include <iostream>
#include <thread>
#include <mutex>
#include <cstdio>
...
void thread_KVStore_server(int thread_index, QpHandler *handler, void *buf, size_t ops,NetParam net_param) {
	...
	uint32_t each_dpu;
	/* V2REF: flag instead of the hard-coded relative path. */
	DPU_ASSERT(dpu_alloc(DPU_NUM, "nrThreadPerPool=8", &set1));
	DPU_ASSERT(dpu_load(set1, DPU_BINARY_PATH.c_str(), NULL));

	DPU_ASSERT(dpu_alloc(DPU_NUM, "nrThreadPerPool=8", &set2));
	DPU_ASSERT(dpu_load(set2, DPU_BINARY_PATH.c_str(), NULL));

	DPU_ASSERT(dpu_alloc(DPU_NUM, "nrThreadPerPool=8", &set3));
	DPU_ASSERT(dpu_load(set3, DPU_BINARY_PATH.c_str(), NULL));


	// //* init KV storage
	/* V2REF: heap instead of a stack VLA so the full 32 MiB table fits. */
	size_t table_bytes = MAX_HASH_ENTRY_NUM * sizeof(struct kv_storage);
	struct kv_storage *key_entry_array =
		(struct kv_storage *)malloc(table_bytes);
	assert(key_entry_array != NULL);
	for(uint64_t i = 0; i < MAX_HASH_ENTRY_NUM; ++i) {
	...
	DPU_ASSERT(dpu_push_xfer(set1, DPU_XFER_TO_DPU, "key_entry_array",0, table_bytes, DPU_XFER_DEFAULT));

	DPU_FOREACH(set2, dpu, each_dpu){
		DPU_ASSERT(dpu_prepare_xfer(dpu,key_entry_array));
	}
	DPU_ASSERT(dpu_push_xfer(set2, DPU_XFER_TO_DPU, "key_entry_array",0, table_bytes, DPU_XFER_DEFAULT));

	DPU_FOREACH(set3, dpu, each_dpu){
		DPU_ASSERT(dpu_prepare_xfer(dpu,key_entry_array));
	}
	DPU_ASSERT(dpu_push_xfer(set3, DPU_XFER_TO_DPU, "key_entry_array",0, table_bytes, DPU_XFER_DEFAULT));

	/* V2REF */
	std::ofstream v2ref_dump;
	uint64_t v2ref_records = 0;
	if (!DUMP_PATH.empty())
		v2ref_dump.open(DUMP_PATH, std::ios::binary | std::ios::trunc);
	...
	for (uint64_t i = 0; i < ops; i+= REQUEST_PER_DPU) {
		char tmp1[VALUE_SIZE*REQUEST_PER_DPU],tmp2[VALUE_SIZE*REQUEST_PER_DPU],tmp3[VALUE_SIZE*REQUEST_PER_DPU];
			...
			*(uint64_t*)(resp +REQUEST_PER_DPU * VALUE_SIZE) = 1; // mark response as valid
			...
			t[3] = get_tscp();
			/* V2REF: dump the (key, value) responses the
			 * original discarded, and cross-check the three
			 * identical dpu_set copies. */
			if (v2ref_dump.is_open()) {
				assert(memcmp(tmp1, tmp2,
					      REQUEST_PER_DPU * VALUE_SIZE) == 0);
				assert(memcmp(tmp1, tmp3,
					      REQUEST_PER_DPU * VALUE_SIZE) == 0);
				for (int j = 0; j < REQUEST_PER_DPU; ++j) {
					uint64_t key = 0, value = 0;
					memcpy(&key,
					       get_msg + request_offset +
						       sizeof(uint64_t) +
						       (size_t)j * KEY_SIZE,
					       sizeof(key));
					memcpy(&value,
					       tmp1 + (size_t)j * VALUE_SIZE,
					       sizeof(value));
					v2ref_dump.write((const char *)&key,
							 sizeof(key));
					v2ref_dump.write((const char *)&value,
							 sizeof(value));
					++v2ref_records;
				}
			}
	...
	if (v2ref_dump.is_open()) {
		v2ref_dump.close();
		std::cout << "V2REF dumped " << v2ref_records
			  << " (key,value) pairs to " << DUMP_PATH << std::endl;
	}
	free(key_entry_array);
...
void benchmark(NetParam &net_param) {
	...
	/* V2REF: the response region still starts at
	 * MAX_HASH_ENTRY_NUM * sizeof(get_message) (unchanged layout), but
	 * the tail is sized from the actual request stream instead of 10x
	 * the table. */
	BUF_SIZE = MAX_HASH_ENTRY_NUM * sizeof(get_message) +
		   10 * (size_t(1) * ITERATIONS * NUM_PACK) *
			   (sizeof(get_message) + sizeof(response_message)) +
		   4096;
...
/* V2REF */
DEFINE_string(dpuBinary, DPU_BINARY_USER, "DPU binary path");
DEFINE_string(dumpPath, "", "dump (key,value) responses to this file");
...
int main(int argc, char *argv[]) {
	...
	/* V2REF */
	DPU_BINARY_PATH = FLAGS_dpuBinary;
	DUMP_PATH = FLAGS_dumpPath;
