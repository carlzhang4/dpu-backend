			       ...
			       pimnic_pe_set_t **out)
{
	...
	rc = ctx->ops.alloc(ctx->user, nr_pes, topology, ctx->profile.c_str(),
			    &set->platform_set);
	if (rc != 0) {
		delete set;
		return rc;
...
static int alloc_pe_with_profile(pimnic_ctx_t *ctx, uint32_t nr_pes,
				 const pimnic_topology_t *topology,
				 const char *profile, pimnic_pe_set_t **out)
{
	if (ctx == nullptr || out == nullptr || nr_pes == 0 ||
	    (nr_pes % 64u) != 0)
		return -EINVAL;
	...
	set->nr_pes = nr_pes;
	set->topology = *topology;
	const char *selected = profile != nullptr ? profile : ctx->profile.c_str();
	rc = ctx->ops.alloc(ctx->user, nr_pes, topology, selected,
			    &set->platform_set);
	if (rc != 0) {
		delete set;
		return rc;
				   ...
				   pimnic_pe_set_t **out)
{
	...
	return alloc_pe_with_profile(default_ctx, nr_pes, &topology, profile,
				     out);
}
...
{
	if (set == nullptr || host_src == nullptr || dims == nullptr ||
	    bytes_per_pe == 0 ||
