...
__host volatile uint32_t gnn_completed;
__host volatile uint32_t gnn_digest;
	...
	for (uint32_t cycle = 0; cycle < gnn_cycles; ++cycle) {
		...
		do {
			rc = pimnic_collective_enter(
				&pe, collective,
				PIMNIC_MRAM_ABSOLUTE(&gnn_contribution[0]),
				sizeof(output));
		} while (rc == -EAGAIN);
		if (rc != 0)
			return 1;
		for (;;) {
			...
			if (rc == 0 || cqe.type == PIMNIC_CQ_SEND_DONE)
				continue;
			mram_read(&rx_data[cqe.mram_off],
				  input, sizeof(input));
			for (uint32_t byte = 0; byte < sizeof(input); ++byte) {
				digest ^= ((uint8_t *)input)[byte];
				digest *= 16777619u;
			}
			gnn_digest = digest;
			gnn_completed = cycle + 1u;
